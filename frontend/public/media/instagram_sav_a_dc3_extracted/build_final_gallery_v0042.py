#!/usr/bin/env python3
"""
build_final_gallery_v0042.py - Complete Gallery with Centered Templates, Grid Overlay, Snap Controls
Compliant with all 6 fixes from JSON list:
1. Template centering and symmetrical margins
2. Show grid lines with toggle and color picker
3. Disable snapping (independent toggle control)
4. Images remain movable after template application
5. Template-aware snap to grid sections
...
OUTPUT_HTML = Path("index_v0042.html")
...
<title>Mr. Douglas Gallery v0042 - Centered Templates + Grid + Snap</title>
<style>
...
<div class="menu-bar">
    ...
    <div class="menu-item">
        ...
        <div class="menu-dropdown">
            ...
            <div class="menu-option">Grid: Show/Hide, Snap toggle, Color picker</div>
        </div>
    </div>
</div>
...
<div id="storyboardModal" class="storyboard-modal">
    <div class="storyboard-container">
        <div style="display:flex;justify-content:space-between;">
            <h3>Storyboard Builder - Centered Templates + Grid + Snap</h3>
        ...
        <div class="snap-controls" id="snapControlsContainer">
            <button id="snapToGridBtn" style="background:#8b5cf6;">🔲 Snap: OFF</button>
...
<script>
// ============================================================================
// PATH: index_v0042.html - Complete Implementation
// FIX #1: Template centering and symmetrical margins
// FIX #2: Show grid lines with toggle and color picker
// FIX #3: Disable snapping (independent toggle control)
// FIX #4: Images remain movable after template application
// FIX #5: Template-aware snap to grid sections
...
// Template-aware snap variables (Fix #5)
...
// ========== TEMPLATES (Fix #1 - Centered with Symmetrical Margins) ==========
var TEMPLATES = {
...
function applyLayoutTemplate(templateId) {
    ...
    if(tpl.isCenter) {
        ...
        for(var i=0; i<storyboardImages.length; i++) {
            ...
            obj.scale(scale);
            var scaledWidth = obj.width * scale;
            obj.set({ left: colX[colIdx] + (colWidth - scaledWidth) / 2, top: colHeights[colIdx], angle: 0, hasControls: true, hasBorders: true });
    ...
    canvas.renderAll();
    saveStoryboardState();
    addLog('Template applied: ' + tpl.name + ' to ' + storyboardImages.length + ' images (centered, symmetrical margins)', false);
    showToast('Template applied: ' + tpl.name + ' (' + storyboardImages.length + ' images)', false);
    
    // Recalculate template cell positions for snapping
...
function createGridOverlay() {
    ...
    addLog('Grid overlay enabled: ' + gridSize + 'px cells, color ' + gridColor, false);
}
...
// ========== SNAP TO GRID (Fix #3 & #5 - Template-Aware) ==========
function loadSnapState() {
    var saved = localStorage.getItem('snap_to_grid_enabled_v2');
    if(saved !== null) {
        snapToGridEnabled = saved === 'true';
    }
    var savedSize = localStorage.getItem('snap_grid_size_v2');
    if(savedSize) {
        snapGridSize = parseInt(savedSize);
        document.getElementById('snapGridSizeInput').value = snapGridSize;
    }
    if(snapToGridEnabled) {
        enableSnapToGrid();
        document.getElementById('snapToGridBtn').textContent = '🔲 Snap: ON';
...
function calculateTemplateCellPositions(templateId) {
    ...
    if(tpl.cols) {
        ...
        var colWidth = availW / cols;
        for(var c=0; c<cols; c++) {
            var cellCenterX = margin + c * colWidth + colWidth / 2;
            positions.push({ x: cellCenterX, y: margin + 100, width: colWidth, height: 200 });
        }
    }
...
function snapToTemplateCell(obj) {
    if(!snapToGridEnabled) return false;
...
function enableSnapToGrid() {
    ...
    canvas.on('object:moving', function(e) {
        if(snapToGridEnabled) {
            ...
            if(!snapped) {
                var snapX = Math.round(obj.left / snapGridSize) * snapGridSize;
                var snapY = Math.round(obj.top / snapGridSize) * snapGridSize;
    ...
    canvas.on('object:scaling', function(e) {
        if(snapToGridEnabled) {
            var obj = e.target;
            var snapScaleX = Math.round(obj.scaleX * snapGridSize / obj.width) * obj.width / snapGridSize;
            var snapScaleY = Math.round(obj.scaleY * snapGridSize / obj.height) * obj.height / snapGridSize;
            obj.set({ scaleX: snapScaleX, scaleY: snapScaleY });
        }
    });
    localStorage.setItem('snap_to_grid_enabled_v2', 'true');
    addLog('Snap to grid enabled with ' + snapGridSize + 'px grid', false);
}
...
function disableSnapToGrid() {
    ...
    localStorage.setItem('snap_to_grid_enabled_v2', 'false');
    addLog('Snap to grid disabled', false);
}

function toggleSnapToGrid() {
    if(snapToGridEnabled) {
        disableSnapToGrid();
        document.getElementById('snapToGridBtn').textContent = '🔲 Snap: OFF';
        ...
        document.getElementById('snapToGridBtn').textContent = '🔲 Snap: ON';
        document.getElementById('snapToGridBtn').style.background = '#10b981';
        showToast('Snap to grid ON - snaps to ' + snapGridSize + 'px grid', false);
    }
}

function setSnapGridSize(size) {
    snapGridSize = Math.max(20, Math.min(200, size));
    localStorage.setItem('snap_grid_size_v2', snapGridSize);
    if(snapToGridEnabled) {
        disableSnapToGrid();
        enableSnapToGrid();
    }
...
// ========== SYNC TO STORYBOARD WITH AUTO-TEMPLATE ==========
...
addLog('=== GALLERY v0042 READY ===', false);
addLog('✓ FIX #1: Templates centered with symmetrical margins', false);
addLog('✓ FIX #2: Grid overlay - toggle, color picker, size adjust', false);
addLog('✓ FIX #3: Snap to grid - independent toggle with persistence', false);
addLog('✓ FIX #4: Objects remain movable after template application', false);
addLog('✓ FIX #5: Template-aware snap to cell positions', false);
...
def main():
    print("=" * 70)
    print("MR. DOUGLAS GALLERY v0042 - Centered Templates + Grid + Snap")
    print("=" * 70)
    print("\n✅ All 6 fixes from JSON list implemented:")
    print("   1. Templates centered with symmetrical margins")
    print("   2. Grid overlay with toggle, color picker, size adjust")
    print("   3. Snap to grid - independent toggle with localStorage persistence")
    print("   4. Objects remain movable after template application (Unlock All button)")
    print("   5. Template-aware snap to template cell positions")
    ...
    print("\n✨ NEW FEATURES in v0042:")
    print("   🎯 Centered Templates: Symmetrical margins, true centering")
    print("   📐 Grid Overlay: Show/hide, color picker (default #334155), size adjust (20-200px)")
    print("   🔲 Snap to Grid: Independent toggle, persistent state, template-aware cell snapping")
    print("   🔓 Unlock All: Restores movement to any locked objects")
    print("   🎨 Template-Aware Snap: Snaps to template cell centers, not just arbitrary grid lines")
