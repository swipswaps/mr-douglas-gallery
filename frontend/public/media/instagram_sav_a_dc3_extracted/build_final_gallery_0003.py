...
# -*- coding: utf-8 -*-
"""
FINAL GALLERY BUILDER – FABRIC.JS STORYBOARD WITH PROPER LAYOUT
- Author names extracted from comments
- No timeline
- Checkboxes + sync + auto-add
- Storyboard: grid/2col/3col/bigSmall/center layouts with no overlap
- Resize handles, drag, localStorage, new tab, 300 DPI export
"""
...
# ---------- HTML template with corrected Fabric.js layout ----------
template = """
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <script>
        // All posts data
        const allPosts = __POSTS_JSON__;
        const DISPLAY_MODE = "__DISPLAY_MODE__";
        ...
        // ========== FABRIC.JS STORYBOARD WITH CORRECTED LAYOUT ==========
        let canvas = null;
        let storyboardImages = []; // each: { src, fabricObj, originalWidth, originalHeight }
        ...
        // Corrected layout – no overlap, proper alignment
        function applyLayout(templateName) {
            ...
            if (templateName === 'center') {
                // single image centered (uses only first image)
                ...
                for (let i = 0; i < cnt; i++) {
                    ...
                    const col = i % cols;
                    if (col === 0 && i !== 0) {
                        // move to next row: add previous row's max height + margin
                        const prevRowIndex = i - cols;
                        const prevHeight = (storyboardImages[prevRowIndex]?.fabricObj.height * storyboardImages[prevRowIndex]?.fabricObj.scaleY) || 0;
                        y += prevHeight + margin;
                    }
                    let scale = Math.min(cellW / obj.width, 300 / obj.height);
                    obj.scale(scale);
                ...
                big.scale(scaleBig);
                big.set({ left: margin, top: margin + (availH - big.height * scaleBig) / 2 });
                ...
                for (let i = 1; i < cnt; i++) {
                    ...
                    obj.set({ left: margin + big.width * scaleBig + margin, top: y });
                    y += obj.height * scaleSmall + margin;
                }
            }
            canvas.renderAll();
            saveToLocalStorage();
        ...
        function initCanvas() {
            ...
            canvas.renderAll();
            // Re-add existing fabric objects after canvas init
            for (let item of storyboardImages) {
        ...
        function openStoryboardNewTab() {
            ...
            w.document.write(`<!DOCTYPE html><html><head><title>Mr. Douglas Storyboard</title><style>body{margin:0;background:#0f172a;color:white;font-family:monospace;}canvas{display:block;margin:20px auto;border:2px solid #475569;background:white;}.controls{text-align:center;padding:10px;}button{margin:5px;padding:8px 16px;background:#3b82f6;border:none;color:white;border-radius:8px;cursor:pointer;}</style><script src="https://cdnjs.cloudflare.com/ajax/libs/fabric.js/5.3.0/fabric.min.js"><\\/script></head><body><div class="controls"><button id="exportBtn">⬇ Export PNG (10800×14400)</button><button id="closeBtn" onclick="window.close()">✖ Close</button></div><canvas id="storyboardCanvasNew" width="1080" height="1440"></canvas><script>
                ...
                function drawCanvas() {
                    ...
                    for(let i=0;i<images.length;i++){
                        const img = images[i];
                        const col = i%cols;
                        if(col===0 && i!==0) y += (images[i-cols]?.height * images[i-cols]?.scaleY || 0) + margin;
                        let maxH = 200;
...
html_content = template.replace("__POSTS_JSON__", json.dumps(posts, ensure_ascii=False)).replace("__DISPLAY_MODE__", DISPLAY_MODE)
...
print(f"   Author display: {DISPLAY_MODE}")
print("\n💡 Start server: python -m http.server 8000")
print(f"   Then open http://localhost:8000/{OUTPUT_HTML.name}")
print("\n✨ Fixed storyboard layout:")
print("   - Images auto‑arranged in grid, two columns, three columns, big+small, or centered")
print("   - No overlap, margins respected")
print("   - Resize handles, drag, remove via thumbnails")
print("   - 300 DPI export works")