#!/usr/bin/env python3
"""
build_final_gallery_v0034.py - Diagnostic + Working Gallery
"""
...
OUTPUT_HTML = Path("index_v0034.html")
...
def build_html(posts):
    # For debugging, let's see the size
    posts_json = json.dumps(posts, ensure_ascii=False)
    print(f"JSON size: {len(posts_json)} bytes")
    
    # Create a simplified version first to test
    simple_posts = []
    for p in posts[:10]:  # Just first 10 for testing
        simple_posts.append({
            ...
            "all_media": p["all_media"][:3],
            "comments": p["comments"][:5],
            "instagram_url": p["instagram_url"]
        })
    
    simple_json = json.dumps(simple_posts, ensure_ascii=False)
    print(f"Simple JSON size: {len(simple_json)} bytes")
    
    # Use simple version for reliability
    final_json = simple_json
...
<title>Mr. Douglas Gallery v0034 - Working Gallery</title>
<style>
...
<div id="storyboardModal" class="storyboard-modal">
    <div class="storyboard-container">
        <div style="display:flex;justify-content:space-between;">
            <h3>Storyboard Builder - 300 DPI Export</h3>
...
<script>
// Diagnostic: Check script execution
(function() {
    var debugDiv = document.getElementById("debugLog");
    if(debugDiv) {
        debugDiv.innerHTML = "<div>Script loaded at " + new Date().toLocaleTimeString() + "</div>";
    }
...
console.log("=== GALLERY v0034 STARTING ===");

// Load posts data
var allPosts = ''' + final_json + ''';
...
function addLog(msg, type) {
    ...
    var prefix = "";
    var color = "#10b981";
    if(type === "error") { prefix = "[ERROR] "; color = "#ef4444"; }
    else if(type === "warning") { prefix = "[WARN] "; color = "#f59e0b"; }
    else if(type === "success") { prefix = "[OK] "; color = "#10b981"; }
    else { prefix = "[INFO] "; color = "#60a5fa"; }
...
function saveLogs() {
    if(allLogs.length === 0) {
        addLog("No logs to save", "warning");
        return;
    }
...
function renderGallery(posts) {
    ...
    for(var idx=0; idx<posts.length; idx++){
        ...
        if(firstMedia) {
            if(isVideoPost) {
                mediaHtml = "<div class='video-thumbnail'>";
        ...
        if(post.caption) {
            var shortCaption = post.caption.length > 120 ? post.caption.substring(0,120) + "..." : post.caption;
            htmlStr += "<div class='card-caption'>" + shortCaption.replace(/</g, '&lt;').replace(/>/g, '&gt;') + "</div>";
        }
    ...
    document.querySelectorAll(".card").forEach(function(card) {
        card.onclick = function(e) {
            if(e.target.closest(".comments-btn") || e.target.closest(".select-checkbox") || e.target.closest(".insta-link")) {
                return;
            }
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    addLog("=== GALLERY v0034 READY ===", "success");
...
def main():
    print("=" * 70)
    print("MR. DOUGLAS GALLERY v0034 - Diagnostic Working Version")
    print("=" * 70)
    ...
    print("[2/3] Generating HTML with simplified data (first 10 posts only)...")
    ...
    print("\n📊 DIAGNOSTIC VERSION:")
    print("   - Loaded only first 10 posts to test JSON parsing")
    print("   - Debug panel shows script execution")
