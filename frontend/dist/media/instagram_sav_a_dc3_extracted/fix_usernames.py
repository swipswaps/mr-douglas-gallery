...
Auto-detects the username column.
"""
...
if not DB_PATH.exists():
    ...
    username_col = None
    for candidate in ['username', 'author', 'from_user', 'owner', 'user']:
        if candidate in columns:
            username_col = candidate
            break
    if not username_col:
        print("No username column found in database. Available columns:", columns)
        exit(1)
    print(f"Using column '{username_col}' for usernames.")
    
    cursor.execute(f"SELECT shortcode, {username_col} FROM posts")
