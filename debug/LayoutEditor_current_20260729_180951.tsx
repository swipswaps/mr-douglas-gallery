...
// ===== GRID OVERLAY =====
const GridOverlay = ({ size, width, height }: { size: number; width: number; height: number }) => {
...
// ===== DRAGGABLE THUMBNAIL =====
const DraggableThumb: React.FC<{ post: Post; isSelected: boolean; onToggle: (e?: React.MouseEvent) => void; onDoubleClick: () => void }> = ({ post, isSelected, onToggle, onDoubleClick }) => {
...
const DetailModal: React.FC<{ post: Post; onClose: () => void }> = ({ post, onClose }) => {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.85)', zIndex: 10000, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: SPACING.md }} onClick={onClose}>
      <div style={{ background: COLORS.dark, padding: SPACING.xl, borderRadius: '8px', maxWidth: '90vw', maxHeight: '90vh', overflow: 'auto', color: COLORS.text }} onClick={(e) => e.stopPropagation()}>
        ...
        <button onClick={onClose} style={{ marginTop: SPACING.md, background: COLORS.border, border: 'none', color: COLORS.text, padding: `${SPACING.sm} ${SPACING.lg}`, borderRadius: '4px', cursor: 'pointer' }}>Close</button>
      </div>
    </div>
  );
};
...
// ===== TOOLBAR BUTTON =====
const ToolbarButton: React.FC<{ onClick: () => void; icon: string; label: string; disabled?: boolean; variant?: 'primary' | 'secondary' | 'success' | 'warning' | 'danger' | 'default' }> = ({ onClick, icon, label, disabled = false, variant = 'default' }) => {
  const bgColors = {
...
// ===== MAIN COMPONENT =====
const LayoutEditor = ({ posts, onBack }: { posts: Post[]; onBack: () => void }) => {
  // --- Load settings ---
  const loadSettings = () => {
    try {
  ...
  // Global mouseup listener to ensure resize state is always cleared
  useEffect(() => {
  ...
  const onCanvasMouseUp = (_e: React.MouseEvent) => {
    ...
    // Always reset resize state on mouse up
    setResizeTargetId(null);
    setResizeStart(null);
    setResizeCorner(null);
  // --- Drop from sidebar ---
    ...
    <DndProvider backend={HTML5Backend}>
      <div style={{ width: '100vw', height: '100vh', background: COLORS.dark, color: COLORS.text, display: 'flex', flexDirection: 'column', fontFamily: FONT.family }}>
        ...
        <div style={{ background: COLORS.darker, padding: SPACING.sm, borderBottom: `1px solid ${COLORS.border}`, maxHeight: '80px', overflow: 'auto', fontSize: FONT.sizeSmall, color: COLORS.textMuted, display: 'flex', flexWrap: 'wrap', gap: SPACING.xs }}>
          {(logs as string[]).map((log: string, idx: number) => (
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: SPACING.sm, display: 'flex', gap: SPACING.sm }}>
          ...
          {/* Canvas */}
          <div style={{ flex: 1, position: 'relative', background: COLORS.canvasBg, borderRadius: '8px', overflow: 'auto' }}
            ...
            <DropZone onDrop={handleDrop}>
              <div
                ref={containerRef}
                style={{ position: 'relative', width: pageWidth, height: pageHeight, background: backgroundType === 'color' ? backgroundColor : (backgroundType === 'transparent' ? 'transparent' : 'inherit'), margin: '0 auto', cursor: 'default' }}
              ...
              >
                ...
                {isDragSelecting && dragSelectStart && dragSelectEnd && (
                  <div style={{ position: 'absolute', left: Math.min(dragSelectStart.x, dragSelectEnd.x), top: Math.min(dragSelectStart.y, dragSelectEnd.y), width: Math.abs(dragSelectEnd.x - dragSelectStart.x), height: Math.abs(dragSelectEnd.y - dragSelectStart.y), border: `1px dashed ${COLORS.primary}`, background: 'rgba(225,48,108,0.1)', pointerEvents: 'none' }} />
                )}
                {items.map((item) => {
                  ...
                  if (item.type === 'image' && item.post) {
                    ...
                    return (
                      <div
                        ...
                        style={{ position: 'absolute', left: item.x, top: item.y, width: item.width, height: item.height, cursor: 'grab', border: borderStyle, overflow: 'hidden', background: '#fff' }}
                        onMouseDown={(e) => { e.stopPropagation(); onItemMouseDown(e, item.id); }}
                        onDoubleClick={() => item.post && openDetail(item.post)}
                      >
                        <img src={imgUrl} alt={item.postId} style={{ width: '100%', height: '100%', objectFit: 'contain', pointerEvents: 'none' }}
                    ...
                    return (
                      <div key={item.id} style={{ position: 'absolute', left: item.x, top: item.y, width: item.width, height: item.height, cursor: 'grab', border: borderStyle, background: item.backgroundColor || 'transparent', overflow: 'hidden', padding: '4px' }}
                           onMouseDown={(e) => { e.stopPropagation(); onItemMouseDown(e, item.id); }}
                           onDoubleClick={() => { const newText = prompt('Edit text:', item.text || ''); if (newText !== null) updateSelected({ text: newText }); }}>
                        <div style={{ fontFamily: item.fontFamily || 'Arial', fontSize: item.fontSize || 24, color: item.color || '#ffffff', fontWeight: 'bold', width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', textShadow: item.strokeWidth ? `0 0 ${item.strokeWidth}px ${item.strokeColor || '#000000'}` : 'none' }}>
