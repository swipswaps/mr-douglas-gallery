#!/usr/bin/env python3
"""
build_final_gallery_v0036.py - FULL WORKING with ALL features + LOGS
"""
...
OUTPUT_HTML = Path("index_v0036.html")
...
def load_posts():
    ...
    if DB_PATH.exists():
        ...
        for row in rows:
            ...
            post['all_media'] = all_media[:10]  # Limit media per post to reduce JSON size
...
def build_html(posts):
    # Split JSON into chunks to avoid parser issues
    posts_json = json.dumps(posts, ensure_ascii=False)
    print(f"JSON size: {len(posts_json)} bytes for {len(posts)} posts")
    
    # Write JSON to separate file for loading
    json_file = Path("posts_data.json")
    json_file.write_text(posts_json, encoding='utf-8')
    print(f"Saved posts data to {json_file}")
...
<title>Mr. Douglas Gallery v0036 - FULL FEATURES + LOGGING</title>
<style>
...
<div class="debug-panel" id="debugPanel">
    <div class="debug-header">
        <strong>🔍 Debug Console - Logs Auto-Save</strong>
        <span>
    ...
    <div class="storyboard-container">
        ...
        <div class="storyboard-controls">
            <div style="display:flex;gap:10px;margin-bottom:10px;flex-wrap:wrap;">
                ...
                <input type="file" id="logoUpload" accept="image/svg+xml" style="background:#334155;color:white;border:none;padding:4px 8px;border-radius:4px">
                <button id="addLogoBtn" style="background:#10b981;border:none;color:white;padding:4px 12px;border-radius:4px;cursor:pointer">Add Logo</button>
                <button id="removeLogoBtn" style="background:#ef4444;border:none;color:white;padding:4px 12px;border-radius:4px;cursor:pointer">Remove Logo</button>
            </div>
...
function addLog(msg, type) {
    ...
    // Auto-save to localStorage every 10 logs
    if(allLogs.length % 10 === 0) {
        try {
            localStorage.setItem("gallery_logs_backup", JSON.stringify(allLogs.slice(-100)));
        } catch(e) {}
    }
}
...
function saveLogs() {
    ...
    addLog("Saved " + allLogs.length + " logs to file", "success");
}
...
function initCanvas() {
    ...
    var savedBg = localStorage.getItem("storyboard_bg_color");
    if(savedBg) {
...
function saveStoryboardState() {
    ...
    localStorage.setItem("storyboard_bg_color", canvas.backgroundColor);
    addLog("Storyboard state saved", "info");
}

function loadStoryboardState() {
    var saved = localStorage.getItem("storyboard_images");
    if(saved) {
        var images = JSON.parse(saved);
        addLog("Loading " + images.length + " saved images", "info");
        for(var i=0;i<images.length;i++) {
            fabric.Image.fromURL(images[i].src, function(img) {
                if(!img) return;
                img.set({ left: images[i].left, top: images[i].top, scaleX: images[i].scaleX, scaleY: images[i].scaleY, hasControls: true, hasBorders: true });
                storyboardImages.push({ src: images[i].src, fabricObj: img });
                canvas.add(img);
                canvas.renderAll();
                updateThumbnails();
                updateStoryboardBadge();
...
function exportHighQuality() {
    ...
    addLog("Starting 300 DPI export (10800x14400)...", "info");
...
function loadPostsData() {
    addLog("Loading posts data from posts_data.json...", "info");
    fetch('posts_data.json')
        .then(response => response.json())
        .then(data => {
            allPosts = data;
            addLog("Loaded " + allPosts.length + " posts successfully", "success");
            initCanvas();
            loadStoryboardState();
            renderGallery(allPosts);
            addLog("=== GALLERY v0036 READY ===", "success");
            addLog("✅ All features: Background color, SVG logo, Templates, 300 DPI export", "info");
            addLog("💾 Click 💾 to save logs to file", "info");
        })
        .catch(error => {
            addLog("ERROR loading posts data: " + error, "error");
            document.getElementById("galleryGrid").innerHTML = "<div style='text-align:center;padding:3rem;color:#ef4444'>Failed to load posts data. Check console.</div>";
        });
}

document.addEventListener("DOMContentLoaded", function() {
    addLog("DOM ready, initializing...", "info");
    ...
    if(addLogoBtn) {
        addLogoBtn.addEventListener("click", function() {
            ...
            if(!file) { showToast("Select an SVG file first", true); return; }
...
def main():
    print("=" * 70)
    print("MR. DOUGLAS GALLERY v0036 - FULL FEATURES + EXTERNAL JSON")
    print("=" * 70)
    
    print("\n[1/4] Loading ALL posts...")
    posts = load_posts()
    posts = add_historic_images(posts)
    print(f"Loaded {len(posts)} posts total")
    
    print("[2/4] Saving posts to external JSON file...")
    posts_json = json.dumps(posts, ensure_ascii=False)
    json_file = Path("posts_data.json")
    json_file.write_text(posts_json, encoding='utf-8')
    print(f"Saved {len(posts_json)} bytes to {json_file}")
    ...
    print("   📸 300 DPI Export (10800 x 14400 pixels)")
    print("   🎨 Background color picker")
    print("   🖼️ SVG Logo upload (add/remove)")
    print("   📐 Multiple layout templates (Grid, Center, Masonry)")
    print("   💾 Persistent storyboard (saves to localStorage)")
    print("   🎬 Video thumbnails")
    print("   💬 Comments modal")
    print("   🔍 Image load tracking")
    print("   📊 Debug panel with SAVABLE logs (click 💾)")
    print("   💾 Logs auto-save to localStorage every 10 entries")
