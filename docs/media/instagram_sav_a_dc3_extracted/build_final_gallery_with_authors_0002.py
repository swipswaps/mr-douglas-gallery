...
# -*- coding: utf-8 -*-
"""
FINAL GALLERY BUILDER – WITH AUTHOR NAMES, NO TIMELINE, FIXED STORYBOARD
- Author names extracted from comments (most frequent @mention)
- No timeline section
- Fabric.js storyboard that works: images added before modal opens appear when canvas is initialised
"""
    ...
    <script>
        ...
        function commentHandler(e) {{
            ...
            if (post && post.comments.length) {{
                document.getElementById('commentsList').innerHTML = post.comments.map(c => `<div class="comment-item">💬 ${{c}}</div>`).join('');
        ...
        function attachCarouselListeners() {{
            document.querySelectorAll('.carousel-item, .carousel-video-placeholder').forEach(el => {{
                el.removeEventListener('click', carouselHandler);
                el.addEventListener('click', carouselHandler);
            }});
        }}
        ...
        document.getElementById('galleryGrid').addEventListener('click', (e) => {{
            ...
            if (card && !e.target.closest('.carousel-item') && !e.target.closest('.carousel-video-placeholder') && !e.target.closest('.comments-btn') && !e.target.closest('a') && !e.target.closest('.select-checkbox')) {{
                let media = null;
        ...
        // ========== STORYBOARD (FIXED: images persist when canvas initialises later) ==========
        ...
        async function addImageToStoryboard(src, silent=false) {{
            ...
            try {{
                ...
                const newItem = {{
                    ...
                    left: 50, top: 50, scaleX: defW / imgEl.width, scaleY: defH / imgEl.height,
                    fabricObject: null
                }};
                storyboardImages.push(newItem);
                // If canvas already exists, add the fabric object now
                if (displayCanvas) {{
        ...
        async function addMultipleImages(srcList) {{
            ...
            if (added && displayCanvas) applyTemplate('grid');
        }}
        ...
        function updateStoryboardThumbnails() {{
            ...
            document.querySelectorAll('.storyboard-thumb').forEach(thumb => {{
                thumb.addEventListener('click', () => {{
                    ...
                    if (!isNaN(idx)) {{
                        if (displayCanvas && storyboardImages[idx].fabricObject)
        ...
        function initCanvas() {{
            ...
            if (!canvas) return;
            if (displayCanvas) displayCanvas.dispose();
            ...
            displayCanvas.on('object:modified', (e) => {{
                ...
                if (idx !== -1) {{
                    storyboardImages[idx].left = obj.left;
                    storyboardImages[idx].top = obj.top;
                    storyboardImages[idx].scaleX = obj.scaleX;
                    storyboardImages[idx].scaleY = obj.scaleY;
                }}
            }});
            // Re‑add all existing images to the new canvas
            for (let item of storyboardImages) {{
                if (!item.fabricObject) {{
                    const fimg = new fabric.Image(item.imgElement, {{
                        left: item.left, top: item.top,
                        scaleX: item.scaleX, scaleY: item.scaleY,
                        hasControls: true, lockRotation: true
                    }});
                    item.fabricObject = fimg;
                    displayCanvas.add(fimg);
                }} else {{
                    displayCanvas.add(item.fabricObject);
                }}
            }}
            displayCanvas.renderAll();
            updateStoryboardThumbnails();
