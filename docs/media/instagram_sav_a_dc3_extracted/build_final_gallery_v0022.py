#!/usr/bin/env python3
"""
build_final_gallery_v0022.py - Storyboard with Edge-Aware Super Resolution
"""
...
OUTPUT_HTML = Path("index_v0022.html")
...
<title>Mr. Douglas Gallery v0022 - Edge-Aware Super Resolution</title>
<style>
...
.debug-panel{position:fixed;bottom:10px;right:10px;background:#1e293b;color:#0f0;font-family:monospace;font-size:10px;padding:8px;border-radius:8px;z-index:9999;max-width:450px;max-height:250px;overflow:auto;opacity:0.9;cursor:move}
.debug-panel.minimized{height:30px;overflow:hidden}
.debug-header{display:flex;justify-content:space-between;margin-bottom:5px;cursor:move}
.debug-close{color:#ef4444;cursor:pointer;margin-left:10px}
.image-status{font-size:9px;color:#94a3b8}
...
<div class="debug-panel" id="debugPanel">
    <div class="debug-header">
        <strong>🔍 Debug & Image Monitor</strong>
        <span>
    ...
    <div id="debugLog"></div>
    <div id="imageStatusLog" style="border-top:1px solid #334155;margin-top:5px;padding-top:5px;"></div>
</div>
...
<div id="storyboardModal" class="storyboard-modal">
    <div class="storyboard-container">
        <div style="display:flex;justify-content:space-between;">
            <h3>Storyboard Builder - Edge-Aware Super Resolution</h3>
...
debugPanel.addEventListener("mousedown", function(e) {
    if(e.target.closest("#debugMinimize") || e.target.closest("#debugClose")) return;
    isDragging = true;
...
function addLog(msg, isError) {
    ...
    if(debugLog.children.length > 50) debugLog.removeChild(debugLog.children[0]);
    console.log(msg);
}
...
function updateImageStatus() {
    ...
    if(failed > 0) {
        html += '<div style="font-size:9px;color:#ef4444">Failed: ' + Object.keys(imageLoadErrors).slice(0,5).join(', ') + 
                (failed > 5 ? '...' : '') + '</div>';
    }
...
function renderGallery(posts) {
    ...
    for(var idx=0; idx<posts.length; idx++){
        ...
        if(pm){ 
            ...
            mediaHtml = "<img class='card-media' src='" + mp + "' loading='lazy' data-path='" + mp + "' onerror='trackImageError(this)'>"; 
        }
...
function trackImageError(img) {
    ...
    if(!imageLoadErrors[src]) {
        ...
        addLog("❌ IMAGE FAILED: " + src.split('/').pop(), true);
        updateImageStatus();
    }
}
...
// Override Image constructor to track loads
var originalImage = window.Image;
window.Image = function() {
    var img = new originalImage();
    img.addEventListener('load', function() { 
        if(img.parentElement && img.classList) trackImageLoad(img); 
    });
    img.addEventListener('error', function() { trackImageError(img); });
    return img;
};
...
// Edge-Aware upscaling - preserves edges better for curves and text
function edgeAwareResample(sourceCanvas, targetWidth, targetHeight) {
    ...
    for (var y = 0; y < targetHeight; y++) {
        ...
        for (var x = 0; x < targetWidth; x++) {
            ...
            // Detect if this region contains an edge
            var edgeStrength = detectEdge(sxInt, syInt, source.data, sourceWidth, sourceHeight);
            
            var r = 0, g = 0, b = 0, a_total = 0;
            var weightTotal = 0;
            
            // Adaptive kernel size: smaller near edges (preserves detail), larger in smooth areas
            ...
            for (var dy = -kernelRadius; dy <= kernelRadius; dy++) {
                ...
                for (var dx = -kernelRadius; dx <= kernelRadius; dx++) {
                    ...
                    var weight = lanczosX * lanczosY;
                    // Boost edge weights for better edge preservation
...
function addImageToStoryboard(src) {
    addLog("Adding: " + src.split('/').pop());
    for(var i=0;i<storyboardImages.length;i++){
        if(storyboardImages[i].src === src){ showToast("Already in storyboard"); return; }
    }
    ...
    img.onload = function() {
        ...
        if(needsUpscale) {
            addLog("Upscaling with " + upscaleMode + " mode from " + img.width + "x" + img.height);
            ...
            var upscaledCanvas;
            if(upscaleMode === "edge") {
                upscaledCanvas = edgeAwareResample(sourceCanvas, upscaleTarget, upscaleHeight);
            } else {
                upscaledCanvas = lanczosResample(sourceCanvas, upscaleTarget, upscaleHeight);
            }
            var finalDataURL = upscaledCanvas.toDataURL("image/png");
            addLog("Upscaled to " + upscaleTarget + "x" + Math.round(upscaleHeight));
            
            fabric.Image.fromURL(finalDataURL, function(fabricImg) {
                if(!fabricImg) { showToast("Failed to load upscaled image"); return; }
                fabricImg.set({ hasControls: true, hasBorders: true, lockRotation: true });
                storyboardImages.push({ src: src, fabricObj: fabricImg, originalWidth: img.width, originalHeight: img.height });
                canvas.add(fabricImg);
                applyLayout();
                updateThumbnails();
                updateStoryboardBadge();
                showToast("Added (" + upscaleMode + " upscaled)");
            });
        } else {
            fabric.Image.fromURL(src, function(fabricImg) {
    ...
    img.onerror = function() {
        ...
        showToast("Failed to load: " + src.split('/').pop());
    };
    img.src = src;
}
...
function exportHighQuality() {
    ...
    addLog("Starting super-resolution export with " + currentUpscaleMode + " mode...");
    ...
    a.download = "storyboard_superres_" + currentUpscaleMode + ".png";
...
addLog("=== READY - Edge-Aware Super Resolution ===");
</script>
</body>
</html>'''
...
def main():
    print("=" * 70)
    print("MR. DOUGLAS GALLERY BUILDER v0022 - Edge-Aware Super Resolution")
    print("=" * 70)
    ...
    print("[2/3] Generating HTML with edge-aware upscaling...")
    ...
    print("\n✨ NEW FEATURES:")
    print("   🔍 Image Monitor - Shows which images fail to load")
    print("   🎨 Edge-Aware Upscaling - Better for curves, text, and logos")
    print("   🔘 Check Missing Images button - Find broken image paths")
    print("   📊 Real-time image load status in debug panel")
