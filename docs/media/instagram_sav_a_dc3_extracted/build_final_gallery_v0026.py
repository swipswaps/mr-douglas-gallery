#!/usr/bin/env python3
"""
build_final_gallery_v0025.py - Fixed lightbox, video player, and comments
"""
...
OUTPUT_HTML = Path("index_v0025.html")
...
<title>Mr. Douglas Gallery v0025 - Fixed Lightbox & Comments</title>
<style>
...
.video-overlay{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);background:rgba(0,0,0,0.7);color:white;padding:12px 16px;border-radius:50%;font-size:24px;pointer-events:none}
</style>
</head>
<body>
...
function openLightbox(mediaItems, startIndex, caption) {
    ...
    addLog("Lightbox opened with " + mediaItems.length + " media items");
}
...
function showLightboxMedia(index) {
    ...
    if(isVideo(media.url)) {
        ...
        addLog("Playing video: " + media.url.split('/').pop());
    } else {
...
function showComments(shortcode) {
    addLog("Loading comments for: " + shortcode);
    var post = null;
    ...
    if(!post || !post.comments || post.comments.length === 0) {
        document.getElementById("commentsList").innerHTML = "<div style='text-align:center;padding:20px;color:#94a3b8'>No comments available</div>";
    } else {
        var html = "";
        for(var i=0;i<post.comments.length;i++) {
            html += "<div class='comment-item'>💬 " + escapeHtml(post.comments[i]) + "</div>";
        }
        document.getElementById("commentsList").innerHTML = html;
        addLog("Displayed " + post.comments.length + " comments");
    }
...
function renderGallery(posts) {
    ...
    for(var idx=0; idx<posts.length; idx++){
        ...
        if(post.all_media.length > 1) {
            ...
            for(var i=0;i<post.all_media.length;i++) {
                ...
                if(isVideo(mediaFile)) {
                    carouselHtml += "<div class='carousel-video-item' data-url='" + mediaUrl + "' data-type='video'>🎬</div>";
                } else {
                    carouselHtml += "<img class='carousel-item' src='" + mediaUrl + "' data-url='" + mediaUrl + "' data-type='image' loading='lazy'>";
                }
            }
            carouselHtml += "</div>";
        }
        ...
        if(firstMedia) {
            if(isVideoPost) {
                mediaHtml = "<div style='position:relative;background:#0f172a;display:flex;align-items:center;justify-content:center;min-height:200px;aspect-ratio:4/3;'><div style='text-align:center'>🎬 Video File<div class='video-overlay'>▶</div></div></div>";
            } else {
        ...
        htmlStr += "<div style='position:relative;width:100%;aspect-ratio:4/3;' class='card-media-container'>" + mediaHtml + "</div>";
        ...
        if(shortCaption) htmlStr += "<div class='card-caption'>" + escapeHtml(shortCaption) + "</div>";
        if(carouselHtml) htmlStr += carouselHtml;
        htmlStr += "<div class='card-footer'>";
        htmlStr += "<a href='" + post.instagram_url + "' target='_blank' class='insta-link' onclick='event.stopPropagation()'>🔗 View on Instagram</a>";
        htmlStr += "<button class='comments-btn' data-shortcode='" + post.shortcode + "'>💬 " + post.comments.length + " comments</button>";
        htmlStr += "</div></div></div>";
    }
...
function attachCarouselListeners() {
    var carouselItems = document.querySelectorAll(".carousel-item, .carousel-video-item");
...
function selectAll() {
    ...
    for(var i=0;i<checkboxes.length;i++) if(!checkboxes[i].checked) checkboxes[i].click();
}
...
function addImageToStoryboard(src) {
    ...
    fabric.Image.fromURL(src, function(img) {
        if(!img) { addLog("Failed to load: " + filename, true); showToast("Failed to load: " + filename); return; }
...
document.querySelector(".modal-close").onclick = function() { document.getElementById("commentsModal").classList.remove("active"); };
...
def main():
    print("=" * 70)
    print("MR. DOUGLAS GALLERY BUILDER v0025 - Fixed Lightbox & Comments")
    print("=" * 70)
    ...
    print("\n✨ FIXES in v0025:")
    print("   🖼️ Lightbox with working navigation (arrows)")
    print("   🎬 Video player loads in same lightbox")
    print("   💬 Comments modal displays all comments")
    print("   📷 Carousel thumbnails for multi-image posts")
    print("   ⌨️ Keyboard navigation (arrow keys, escape)")
