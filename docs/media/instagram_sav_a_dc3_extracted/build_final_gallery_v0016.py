#!/usr/bin/env python3
"""
build_final_gallery_v0016.py

WORKING v0013 gallery WITH integration to existing logging:
- db_logger.py (saves to gallery_errors.db)
- error_server.py (port 8001)
- run_error_server.py (startup script)
"""
...
# Try to import existing logging modules
try:
    from db_logger import init_error_db, log_step, validate_image_path, extract_logs_from_db
    DB_LOGGER_AVAILABLE = True
except ImportError:
    DB_LOGGER_AVAILABLE = False
    print("Warning: db_logger.py not found - logging to database disabled")
...
OUTPUT_HTML = Path("index_v0016.html")
...
def check_error_server():
    """Check if error server is running (for browser logging)"""
    try:
        with urllib.request.urlopen('http://localhost:8001/health', timeout=0.5) as response:
...
def load_posts(db_conn=None):
    ...
    if DB_PATH.exists():
        ...
        if db_conn and DB_LOGGER_AVAILABLE:
            log_step(db_conn, "DB_LOAD", "START", f"Loading from {DB_PATH}")
            ...
            if folder.exists():
                ...
                if db_conn and DB_LOGGER_AVAILABLE:
                    for media_file in all_media:
                        validate_image_path(db_conn, post['folder_name'], media_file, BASE_PATH)
        ...
        if db_conn and DB_LOGGER_AVAILABLE:
            log_step(db_conn, "DB_LOAD", "COMPLETE", f"Loaded {len(posts)} posts")
    return posts

def add_historic_images(posts, db_conn=None):
    ...
    if timeline_folder.exists():
        for img_path in sorted(timeline_folder.glob("*.jpg")):
            ...
            if db_conn and DB_LOGGER_AVAILABLE:
                validate_image_path(db_conn, "timeline", img_path.name, BASE_PATH)
        logger.info(f"Added historic images")
    return posts

def build_html(posts, error_server_available):
    posts_json = json.dumps(posts, ensure_ascii=False)
    
    # v0013 WORKING HTML with logging integration
    html = '<!DOCTYPE html>\n'
    html += '<html lang="en">\n<head>\n'
    html += '<meta charset="UTF-8">\n'
    html += '<meta name="viewport" content="width=device-width, initial-scale=1.0">\n'
    html += '<title>Mr. Douglas Gallery v0016</title>\n'
    ...
    # Debug panel with status indicator
    status_class = "status-on" if error_server_available else "status-off"
    status_text = "🟢 Log Server Connected" if error_server_available else "🔴 Log Server Offline"
    html += f'<div class="debug-panel" id="debugPanel">\n'
    html += f'<strong><span class="status-indicator {status_class}"></span>{status_text}</strong>\n'
    ...
    # Logging functions - send to error_server on port 8001
    ...
    html += '  }\n'
    html += '}\n'
    html += 'addLog("=== GALLERY v0016 STARTED ===", "info");\n'
    html += 'addLog("Log server: " + (serverAvailable ? "CONNECTED" : "OFFLINE"), serverAvailable ? "info" : "warn");\n'
    html += 'addLog("Posts loaded: " + allPosts.length, "info");\n'
    
    # Helper functions (same as v0013)
    ...
    # Event listeners (remainder - same as v0013, will continue in final output)
    ...
    html += '  addLog("[Comments] Loading for post: " + sc, "info");\n'
    ...
    html += '      addLog("[Comments] Displayed " + allPosts[i].comments.length + " comments", "info");\n'
    html += '      return;\n'
    html += '    }\n'
    html += '  }\n'
    ...
    html += '  addLog("[Carousel] Opening: " + media, "info");\n'
    html += '  openLightbox(media, caption);\n'
    html += '}\n'
    html += 'function openLightbox(src, caption){\n'
    html += '  addLog("[Lightbox] Opening: " + src, "info");\n'
    ...
    html += '  if(e.target === document.getElementById("lightbox")){ addLog("[Lightbox] Closed (outside click)", "info"); document.getElementById("lightbox").classList.remove("active"); }\n'
    html += '  if(e.target === document.getElementById("commentsModal")){ addLog("[Comments] Modal closed (outside click)", "info"); document.getElementById("commentsModal").classList.remove("active"); }\n'
    html += '};\n'
    ...
    html += '      }\n'
    html += '    }\n'
    html += '    addLog("[Card] Opening media for: " + card.dataset.shortcode, "info");\n'
    ...
    html += '      addLog("[Checkbox] " + (isChecked ? "Selected" : "Deselected") + ": " + imageSrc, "info");\n'
    ...
    html += '  addLog("[Select] Select All clicked", "info");\n'
    ...
    html += '  addLog("[Select] Deselect All clicked", "info");\n'
    html += '  var checkboxes = document.querySelectorAll(".select-checkbox");\n'
    ...
    html += '  addLog("[Storyboard] Adding image: " + src, "info");\n'
    html += '  return new Promise(function(resolve){\n'
    html += '    fabric.Image.fromURL(src, function(img){\n'
    html += '      if(!img){ if(!silent) showToast("Failed to load image"); addLog("[Storyboard] Failed to load: " + src, "error"); resolve(false); return; }\n'
    ...
    html += '      addLog("[Storyboard] Added successfully", "info");\n'
    html += '      resolve(true);\n'
    ...
    html += '  addLog("[Storyboard] Applying layout: " + templateName, "info");\n'
...
def main():
    print("=" * 60)
    print("MR. DOUGLAS GALLERY BUILDER v0016")
    print("Working gallery + Database logging")
    print("=" * 60)
    
    # Initialize database if available
    db_conn = None
    if DB_LOGGER_AVAILABLE:
        db_conn = init_error_db()
        log_step(db_conn, "START", "INFO", "Gallery build v0016 started")
    
    # Check error server for browser logging
    error_server_available = check_error_server()
    logger.info(f"Error server available: {error_server_available}")
    
    # Load posts with database logging
    posts = load_posts(db_conn)
    posts = add_historic_images(posts, db_conn)
    ...
    # Build HTML
    html = build_html(posts, error_server_available)
    OUTPUT_HTML.write_text(html, encoding='utf-8')
    logger.info(f"Generated {OUTPUT_HTML.resolve()}")
    
    # Extract logs if database was used
    if db_conn and DB_LOGGER_AVAILABLE:
        log_step(db_conn, "COMPLETE", "INFO", f"Build complete. Total posts: {len(posts)}")
        extract_logs_from_db(db_conn)
        db_conn.close()
    ...
    print(f"3. Terminal 3: python3 build_final_gallery_v0016.py")
    print(f"4. Open: http://localhost:8000/index_v0016.html")
    print("\nLOGS ARE SAVED TO:")
    print(f"  - gallery_errors.db (SQLite database)")
    print(f"  - execution_log_export.txt")
    print(f"  - image_validation_export.txt")
    print("\nDebug panel shows live status and sends logs to error server")
