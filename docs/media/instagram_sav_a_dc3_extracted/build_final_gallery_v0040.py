#!/usr/bin/env python3
"""
build_final_gallery_v0040.py - Complete Gallery with Asset Library, 7 Templates, Auto-Sync, Template Persistence
Compliant with all 6 fixes from JSON list:
1. Asset Library with add/remove actions + batch controls
2. 7 templates (Grid 2/3/4, Masonry, Center, Timeline, Polaroid) + preview gallery
3. Sync to Storyboard auto-applies template + progress indicator
4. Asset library syncs with gallery selection changes
5. Clear Canvas preserves asset library (all assets show OFF)
6. Template persistence in saved storyboards
"""
...
OUTPUT_HTML = Path("index_v0040.html")
...
<title>Mr. Douglas Gallery v0040 - Asset Library + 7 Templates</title>
<style>
...
<script>
// ============================================================================
// PATH: index_v0040.html - Complete Implementation
// FIX #1: Asset Library with add/remove actions + batch controls
// FIX #2: 7 templates + preview gallery
// FIX #3: Sync to Storyboard auto-applies template + progress
// FIX #4: Asset library syncs with gallery selection
// FIX #5: Clear Canvas preserves asset library
// FIX #6: Template persistence in saved storyboards
// ============================================================================
...
function applyLayoutTemplate(templateId) {
    ...
    if(tpl.isCenter && storyboardImages.length > 0) {
        var obj = storyboardImages[0].fabricObj;
        var scale = Math.min(availW / (obj.width || 100), availH / (obj.height || 100));
        obj.scale(scale);
        obj.set({ left: margin + (availW - (obj.width * scale))/2, top: margin + (availH - (obj.height * scale))/2 });
        if(storyboardImages.length > 1) {
            for(var i=1; i<storyboardImages.length; i++) {
                canvas.remove(storyboardImages[i].fabricObj);
            }
            storyboardImages = storyboardImages.slice(0,1);
        }
    } 
        ...
        for(var i=0; i<storyboardImages.length; i++) {
            var obj = storyboardImages[i].fabricObj;
            var angle = (i - storyboardImages.length/2) * tpl.angleStep;
            var scale = baseScale * (1 - i * 0.05);
            obj.scale(scale);
            obj.set({ left: centerX - (obj.width * scale)/2, top: centerY - (obj.height * scale)/2, angle: angle });
        }
    }
    else if(tpl.cols) {
        var cols = tpl.cols;
    ...
    canvas.renderAll();
    saveStoryboardState();
    addLog('Template applied: ' + tpl.name, false);
}
...
function renderAssetLibrary() {
    ...
    for(var i=0; i<assetImages.length; i++) {
        ...
        var actionTitle = isOnCanvas ? 'Remove from canvas' : 'Add to canvas';
...
function removeAllAssetsFromCanvas() {
    var removed = 0;
    assetImages.forEach(function(asset) {
        if(canvasImageSrcs.has(asset.src)) {
    ...
    else showToast('No assets on canvas to remove', true);
}

// ========== SYNC TO STORYBOARD WITH AUTO-TEMPLATE (Fix #3) ==========
...
document.getElementById('removeAllAssetsBtn').onclick = removeAllAssetsFromCanvas;
...
addLog("=== GALLERY v0040 READY ===", false);
addLog("✓ 7 templates: Grid 2/3/4, Masonry, Center, Timeline, Polaroid", false);
addLog("✓ Asset library: add/remove assets, batch controls, syncs with selection", false);
addLog("✓ Sync to Storyboard auto-applies template + progress indicator", false);
addLog("✓ Template persistence in saved storyboards", false);
addLog("✓ Clear Canvas preserves assets in library", false);
</script>
</body>
</html>'''
...
def main():
    print("=" * 70)
    print("MR. DOUGLAS GALLERY v0040 - Asset Library + 7 Templates")
    print("=" * 70)
    print("\n✅ All 6 fixes from JSON list implemented:")
    print("   1. Asset Library with add/remove actions + batch controls")
    print("   2. 7 templates + preview gallery")
    print("   3. Sync to Storyboard auto-applies template + progress")
    print("   4. Asset library syncs with gallery selection")
    print("   5. Clear Canvas preserves asset library")
    print("   6. Template persistence in saved storyboards")
    ...
    print("\n✨ NEW FEATURES in v0040:")
    print("   📦 ASSET LIBRARY: Thumbnails with +/− buttons, Add All, Remove All")
    print("   🎨 7 TEMPLATES: Grid2, Grid3, Grid4, Masonry, Center, Timeline, Polaroid")
    print("   🖼️ TEMPLATE GALLERY: Click preview icons to apply layouts")
    print("   ⚡ AUTO-TEMPLATE ON SYNC: Selected template applies automatically after sync")
    print("   💾 TEMPLATE PERSISTENCE: Saved storyboards remember template choice")
    print("   🗑️ CLEAR CANVAS: Removes from canvas but assets stay in library")
    print("   🔄 REAL-TIME ASSET SYNC: Library updates when gallery selection changes")
