#!/usr/bin/env python3
"""
build_final_gallery_v0031.py - COMPLETE WORKING WITH FIXED IDS
"""
...
OUTPUT_HTML = Path("index_v0031.html")
ACCOUNT_OWNER = "sav_a_dc3"
LOG_FILE = Path("gallery_debug.log")
...
<title>Mr. Douglas Gallery v0031 - Fixed IDs</title>
<style>
...
.carousel-video-item{width:60px;height:60px;background:#1e293b;border-radius:8px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:24px}
...
.storyboard-btn{position:fixed;bottom:20px;right:20px;background:#3b82f6;color:white;border:none;border-radius:50px;padding:12px 24px;cursor:pointer;z-index:1000;font-size:1rem}
...
console.log("=== GALLERY v0031 STARTING ===");
...
function renderGallery(posts) {
    ...
    for(var idx=0; idx<posts.length; idx++){
        ...
        if(firstMedia) {
            if(isVideoPost) {
                mediaHtml = "<div style='position:relative;background:#0f172a;display:flex;align-items:center;justify-content:center;height:100%;min-height:200px;'>🎬 VIDEO<div style='position:absolute;font-size:48px;opacity:0.5;'>▶</div></div>";
            } else {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    addLog("=== GALLERY v0031 READY ===");
    addLog("Click 💾 to save logs, select images, use storyboard");
});
</script>
</body>
</html>'''
...
def main():
    print("=" * 70)
    print("MR. DOUGLAS GALLERY v0031 - Fixed All ID Issues")
    print("=" * 70)
    ...
    print("[2/3] Generating HTML with fixed IDs...")
