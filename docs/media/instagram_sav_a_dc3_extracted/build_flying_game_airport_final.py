#!/usr/bin/env python3
"""
Flight simulator with DeLand airport terrain – corrected orientation, fallback grid.
"""
...
<head>
    ...
    <title>Mr. Douglas – DeLand Airport (Final)</title>
    <style>
...
<body>
    <div id="info">
        <strong>✈️ Mr. Douglas – DeLand Airport (Final)</strong><br>
    ...
    <script type="module">
        ...
        window.__testState = {{ airplaneLoaded: false, terrainLoaded: false }};
        ...
        // --- Fallback ground grid (always visible) ---
        const gridHelper = new THREE.GridHelper(1000, 50, 0x88aaff, 0x335588);
        gridHelper.position.y = -5;
        scene.add(gridHelper);

        // --- Load terrain (if file exists) ---
        const stlLoader = new STLLoader();
        stlLoader.load('{terrain_path}', (geometry) => {{
            geometry.computeBoundingBox();
            const box = geometry.boundingBox;
            const scale = 100 / Math.max(box.max.x - box.min.x, box.max.z - box.min.z);
            const material = new THREE.MeshStandardMaterial({{ color: 0x8B5A2B, roughness: 0.7, metalness: 0.1 }});
            const terrain = new THREE.Mesh(geometry, material);
            terrain.scale.set(scale, scale, scale);
            const centerY = (box.min.y + box.max.y) / 2 * scale;
            terrain.position.set(0, -5 - centerY, 0);
            terrain.castShadow = true;
            terrain.receiveShadow = true;
            scene.add(terrain);
            window.__testState.terrainLoaded = true;
            console.log('Terrain loaded');
        }}, undefined, (err) => console.error('Terrain load error (STL missing or corrupt):', err));
        ...
        loader.load('{model_path}',
            (gltf) => {{
                ...
                // Adjust orientation – try different axes based on model
                // If still upside down, change to airplane.rotation.z = -Math.PI/2
                airplane.rotation.x = -Math.PI / 2;   // level nose (may need adjustment)
...
def main():
    ...
    parser.add_argument('-o', '--output', default='flying_game_airport_final.html')
