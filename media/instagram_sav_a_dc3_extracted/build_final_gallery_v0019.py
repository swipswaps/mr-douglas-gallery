#!/usr/bin/env python3
"""
build_final_gallery_v0019.py

= EXACT v0013 WORKING GALLERY + AUTO-SERVER =
- Uses the EXACT same build_html as v0013
- Adds automatic server startup
- NO modifications to working JavaScript
"""
...
OUTPUT_HTML = Path("index_v0019.html")
...
# ========== THIS IS THE EXACT build_html FROM v0013 ==========
# v0013 WORKING - DO NOT MODIFY
def build_html(posts):
    ...
    html += '<title>Mr. Douglas Gallery v0019</title>\n'
    html += '<style>\n'
    ...
    html += 'addLog("=== GALLERY v0019 STARTED ===");\n'
    ...
    html += 'function updateStoryboardBadge(){ var b = document.getElementById("storyboardCountBadge"); if(b) b.innerText = storyboardImages.length; addLog("[Storyboard] Badge updated: " + storyboardImages.length + " images"); }\n'
    ...
    html += '      if(!img){ if(!silent) showToast("Failed to load image"); addLog("[Storyboard] Failed to load: " + src); resolve(false); return; }\n'
    ...
    html += '      addLog("[Storyboard] Added successfully");\n'
    html += '      resolve(true);\n'
    html += '    }, { crossOrigin: "Anonymous" });\n'
    html += '  });\n'
    html += '}\n'
    html += 'function addMultipleImages(srcList){ var added=0; var promises=[]; for(var i=0;i<srcList.length;i++){ promises.push(addImageToStoryboard(srcList[i], true)); } Promise.all(promises).then(function(results){ for(var i=0;i<results.length;i++) if(results[i]) added++; if(added) showToast("Added "+added+" image(s)"); addLog("[Storyboard] Added " + added + " images from batch"); }); }\n'
    html += 'function syncSelectedToStoryboard(){ var srcs = Array.from(selectedSrcs); if(srcs.length===0){ showToast("No images selected"); return; } addLog("[Storyboard] Syncing " + srcs.length + " selected images"); addMultipleImages(srcs); }\n'
    ...
    html += '}\n'
    html += 'function clearAll(){ if(confirm("Clear all images?")){ addLog("[Storyboard] Clearing all images"); for(var i=0;i<storyboardImages.length;i++) canvas.remove(storyboardImages[i].fabricObj); storyboardImages=[]; canvas.renderAll(); updateThumbnails(); localStorage.removeItem(STORAGE_KEY); updateStoryboardBadge(); showToast("Storyboard cleared"); } }\n'
    html += 'function exportStoryboard(){ if(storyboardImages.length===0){ showToast("No images to export"); return; } addLog("[Storyboard] Exporting " + storyboardImages.length + " images at " + TARGET_W + "x" + TARGET_H); var offCanvas = document.createElement("canvas"); offCanvas.width=TARGET_W; offCanvas.height=TARGET_H; var offCtx=offCanvas.getContext("2d"); offCtx.fillStyle="white"; offCtx.fillRect(0,0,TARGET_W,TARGET_H); for(var i=0;i<storyboardImages.length;i++){ var obj=storyboardImages[i].fabricObj; offCtx.drawImage(obj._element, obj.left*SCALE, obj.top*SCALE, obj.width*obj.scaleX*SCALE, obj.height*obj.scaleY*SCALE); } var a=document.createElement("a"); a.download="storyboard.png"; a.href=offCanvas.toDataURL("image/png"); a.click(); addLog("[Storyboard] Export initiated"); }\n'
    html += 'function updateThumbnails(){ var container=document.getElementById("storyboardThumbnails"); if(!container) return; var html=""; for(var i=0;i<storyboardImages.length;i++) html+="<img class=\"storyboard-thumb\" src=\""+storyboardImages[i].src+"\" data-index=\""+i+"\">"; container.innerHTML=html; var thumbs=document.querySelectorAll(".storyboard-thumb"); for(var i=0;i<thumbs.length;i++){ thumbs[i].onclick=function(){ var idx=parseInt(this.dataset.index); if(!isNaN(idx)){ addLog("[Storyboard] Removing image index: " + idx); canvas.remove(storyboardImages[idx].fabricObj); storyboardImages.splice(idx,1); canvas.renderAll(); updateThumbnails(); saveToLocalStorage(); updateStoryboardBadge(); showToast("Image removed"); } }; } }\n'
    ...
    html += 'document.getElementById("openStoryboardBtn").onclick = function(){ addLog("[Storyboard] Opening modal"); document.getElementById("storyboardModal").classList.add("active"); };\n'
    html += 'document.getElementById("closeStoryboardBtn").onclick = function(){ addLog("[Storyboard] Closing modal"); document.getElementById("storyboardModal").classList.remove("active"); };\n'
    ...
    html += 'window.onclick = function(e){ if(e.target === document.getElementById("storyboardModal")){ addLog("[Storyboard] Modal closed (outside click)"); document.getElementById("storyboardModal").classList.remove("active"); } };\n'
    html += 'initCanvas();\n'
...
def main():
    print("=" * 70)
    print("MR. DOUGLAS GALLERY BUILDER v0019")
    print("EXACT v0013 WORKING GALLERY + AUTO-SERVER")
    print("=" * 70)
    print()
    
    print("[1/4] Building gallery (using v0013 HTML)...")
    posts = load_posts()
    ...
    print("[4/4] Generating HTML (EXACT v0013 format)...")
    ...
    print("   http://localhost:8000/index_v0019.html")
    print()
    print("📊 Status:")
    ...
    threading.Thread(target=open_browser, args=('http://localhost:8000/index_v0019.html',), daemon=True).start()
