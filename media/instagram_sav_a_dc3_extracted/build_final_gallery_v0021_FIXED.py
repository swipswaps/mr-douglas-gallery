#!/usr/bin/env python3
"""
build_final_gallery_v0021_FIXED.py - Storyboard with HIGH-QUALITY Lanczos scaling
"""
...
OUTPUT_HTML = Path("index_v0021_fixed.html")
...
<title>Mr. Douglas Gallery - High Quality Storyboard</title>
<style>
...
.debug-panel{position:fixed;bottom:10px;right:10px;background:#1e293b;color:#0f0;font-family:monospace;font-size:10px;padding:8px;border-radius:8px;z-index:9999;max-width:400px;max-height:200px;overflow:auto;opacity:0.85;cursor:move}
...
<div id="storyboardModal" class="storyboard-modal">
    <div class="storyboard-container">
        <div style="display:flex;justify-content:space-between;">
            <h3>Storyboard Builder - High Quality</h3>
        ...
        <div class="storyboard-controls">
            ...
            <button id="exportStoryboardBtn" class="primary">Export High Quality PNG</button>
...
function renderGallery(posts) {
    ...
    for(var idx=0; idx<posts.length; idx++){
        ...
        var mediaHtml = "";
        if(pm){ var mp = getMediaPath(post.folder_name, pm); mediaHtml = "<img class='card-media' src='" + mp + "' loading='lazy'>"; }
...
var TARGET_W = 10800, TARGET_H = 14400;  // 10x scale for 300 DPI (36x48 inches)
...
function addImageToStoryboard(src) {
    ...
    var img = new Image();
    img.crossOrigin = null;  // Same-origin only
    img.onload = function() {
        addLog("Loaded: " + img.width + "x" + img.height);
        
        // Always upscale small images for better quality
        var targetDisplayWidth = PREVIEW_W * 0.33;
        if(img.width < targetDisplayWidth) {
            addLog("Upscaling from " + img.width + "x" + img.height);
            ...
            var upscaledCanvas = lanczosResample(sourceCanvas, upscaleTarget, upscaleHeight);
...
function exportHighQuality() {
    ...
    addLog("Starting high-quality export...");
    showToast("Exporting high quality PNG...");
    ...
    a.download = "storyboard_10800x14400.png";
...
addLog("=== READY - High Quality Storyboard ===");
</script>
</body>
</html>'''
...
def main():
    print("=" * 70)
    print("MR. DOUGLAS GALLERY BUILDER - High Quality Storyboard")
    print("=" * 70)
    ...
    print("[2/3] Generating HTML with Lanczos scaling...")
    ...
    print("\n✨ FEATURES:")
    print("   - Lanczos high-quality upscaling (prevents pixelation)")
    print("   - Draggable & closable debug panel")
    print("   - 10800x14400 PNG export (10x scale for 300 DPI)")
    print("   - Same-origin images (no CORS issues)")
